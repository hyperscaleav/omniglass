...
**Two write paths, one read path.** Estate mutations write their `audit_log` row **in the same transaction** as the change (via the Storage Gateway, so a committed change always has its audit row). **Auth events** (login, logout) fire on read/no-tx paths, so they emit through a separate non-transactional seam and record `resource = 'auth'`. The read side is `GET /audit-log` (newest first, filterable by resource and verb, backward-paged by a `before` timestamp), which resolves each actor and real-actor to a username. It is **admin/owner-only**: `audit` is a **sensitive resource**, so a partial global wildcard (`viewer`'s `*:read`) does **not** confer `audit:read`, only an explicit `audit:read` (admin) or the full `*:*` (owner) does ([ADR-0014](/architecture/decisions/#adr-0014-the-audit-trail-is-a-sensitive-read-not-reached-by-a-partial-global-wildcard)), so a read-only operator cannot see the security trail. A **failed** login (wrong password) is deliberately **not** written to `audit_log`, so an unauthenticated caller cannot flood it; a separate rate-limit / failed-attempt counter is a later slice.
