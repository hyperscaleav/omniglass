...
A scope of entity E includes E **and everything structurally beneath it** (a location -> its systems -> their components -> their datapoints and alarms). The visible set is **parameterized by action**: `visible_set(P, action)` = the union, over **only the grants whose role carries `action`**, of each scope entity plus its descendants. There is no single global visible set. `visible_set(P, read)` is the widest (every grant carries `read`), so it bounds list/get visibility; `visible_set(P, ack)` is narrower, covering only the entities reachable through a grant whose role carries `ack`. A principal can therefore **read** an entity it cannot **act** on: it is in `visible_set(P, read)` but outside `visible_set(P, ack)`. Dynamic-group scopes recompute as membership changes. Each per-action set is bounded by **fleet size (entities)**, not data volume.
