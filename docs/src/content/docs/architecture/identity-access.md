...
OIDC / SAML auth, the node / NATS path, the permission cache, custom-role management, the tenant-policy
lever, the **cross-tier scope cascade** (a location scope covering the systems and components beneath it:
today each estate entity is scoped by its own tier, and only `secret` / `variable` / `interface` / `task`
resolve through their owner), and the **`group` scope kind** (a grant with `scope_kind=group` is refused
with a 422 today). The per-slice breakdown is on [implementation status](/architecture/status/).
...
- **Every credential is time-bounded.** `credential.expires_at` bounds a bearer's lifetime, and
  ...
  **bootstrap token** now expire too: a **90-day default** with a `--ttl` override, hard-capped at **365 days**
  (a `--ttl` above the cap errors), so no eternal secret sits in the field ([ADR-0019](/architecture/decisions/#adr-0019-every-credential-is-time-bounded-token-purpose-not-expiry-shape),
...
The password identifier is the `principal.id` (not the username), so a username change does not invalidate the credential. Service bearer tokens are `ogp_<locator>_<secret>`: a human-readable prefix for secret-scanners and audit clarity, a 4-byte non-secret locator (stored, so a token can be named without its secret), and a 24-byte (192-bit) `crypto/rand` secret; the server only ever stores `sha256(token)`. Cleartext is returned exactly once at mint time. A `node` enrolls with a per-tenant **NATS JWT/nkey** instead: the credential row stores the nkey public key, NATS verifies a signed nonce, and the JWT carries the node's subject permissions (its placement-derived `visible_set`, see [The node path](#the-node-path)).
...
A permission a role **grants** but the universe does **not** contain is a capability that is not yet routed (enforces nothing). These are legitimate but ahead of their subsystems: today the list is `rule:create,update`, `config:create,update`, `alarm:ack,snooze,resolve`, `credential:*`, `role:*` (custom-role editing; `role:read:admin` is routed), `unit:create`, `severity_level:create`, and `source:create` (`interface:*` and `task:*` are routed now, so they left the list). They show as held-nothing on a role blade, which is the honest signal, and a **drift test** requires each such seed grant to sit in an explicit `aheadOfRoutes` allow-list until its route lands.
...
**Build status:** the table above is the target model. Today the **cross-tier expansion is not built**: a `location` scope contains locations only, a `system` scope systems only, and so on, each estate entity scoped by its own tier (only `secret` / `variable` / `interface` / `task` resolve through their owning entity's tier), and a `group` scope is refused with a 422. Both are still Design, per the status note at the top.
...
location:room-42 (>)` lets a field tech add and edit the child locations inside room-42 (and, when the
cross-tier cascade lands, its systems and components too) without being
...
Attempting to remove the last owner (by grant delete, principal delete, principal disable, or role change) raises a check-violation. The Gateway translates this into a **409** with a clear remediation message ("cannot revoke the last owner grant").
...
- **Capability (RBAC) in the API middleware is a FAST-REJECT, never an authorization.** It answers one necessary-but-not-sufficient question: does the action appear in **any** of the principal's grants? If not, 403 before the gateway is ever touched. Answered from an in-process cache (the flattened union of permissions across all grants). It never grants access: passing the fast-reject only means "not categorically forbidden", scope still decides. Routes declare their required permission by registering through `gated(op, "component", "create")`, the one helper that sets the middleware, the OpenAPI stamp, and the registry entry together.
...
- **Non-entity resources** have no entity `E`, so `canDo` cannot scope by owner. Three governance classes:
  ...
  - **Data registries** (`property_type`, `tag`, `unit`, `event_type`, `severity_level`, source): governed by a distinct **`<registry>:create` curator capability** (`property_type:create`, `tag:create`, `unit:create`, `event_type:create`, `severity_level:create`, `source:create`). A registry entry has no owner entity, so the grant's `scope_kind` is irrelevant: the check is simply whether the principal holds the capability. Granting it to a curator role lets a principal mint registry entries **without** IAM admin; a minted entry carries its own `scope` (an org-scoped entry shadows an official one, the [namespace-shadow pattern](/architecture/properties/#key-scope-template-org-official)), and `official`-scoped entries are reserved to `owner` and the boot seed.
...
1. **Role index**: at boot, the `role` table is loaded into a Go map with `inherits` resolved transitively into each role's flattened pattern set. Wildcards and the **`:read` floor are not pre-expanded**: the matcher (`rbac.Set.Allows`) evaluates both per call, so the floor holds wherever the set is consulted. Refreshed on a NATS KV watch keyed on `role` changes.
...
`permissions` is the flat union of the caller's roles' **raw patterns** (wildcards like `principal:*` and `>` ride through unexpanded, and the `:read` floor is not materialized), so a `useCan(...)` check in the web app applies the same pattern-matching rules the server does. It is a **fast-reject / UI hint only**, the union over all grants: it answers "could this principal ever do X anywhere", never "can it do X to **this** entity". List visibility likewise (a row in `GET /alarms` is read-scoped) does **not** imply per-action authority on that row. Per-row action affordances (the ack/snooze button on a specific alarm) must be computed against `visible_set(P, action)` for that target, which the `grants` array drives: `grants` is the source for advanced UI logic (scope chips, deciding per-row actionability, explaining why a button is or is not shown). The server is the only authority regardless; the flat list and the list view are hints, the scoped gateway decides.
