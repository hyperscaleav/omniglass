...
import UnlabelledTables from '../../../components/UnlabelledTables.astro';
import RuleLanguage from '../../../components/RuleLanguage.astro';
import LabelDataMap from '../../../components/LabelDataMap.astro';
...
([#716](https://github.com/hyperscaleav/omniglass/issues/716)). A **root**'s `stem` is the one
...
one ([#716](https://github.com/hyperscaleav/omniglass/issues/716)). `resolved_icon` says what a row
...
three-state sentinel `component_type` uses ([#716](https://github.com/hyperscaleav/omniglass/issues/716)).
...
- **label**, an optional friendly string a human reads ("HQ Boardroom DSP"). Absent is normal:
  `crestron` needs no label. Absent has exactly one spelling, **SQL NULL**:
  the column is nullable, and the rule that keeps the empty string out of it lives in Go at the
  gateway write path, where every other rule of its kind lives
  ([ADR-0118](/architecture/decisions/#adr-0118-the-friendly-string-an-operator-reads-is-a-label-and-unset-is-sql-null)).
  A read projects `coalesce(label, '')`, so nothing above the Storage Gateway has to know. A surface
  with no label renders the name **verbatim**, never re-cased or prettified. The one spelling is
  what lets a registry list say `order by label nulls last, name` and mean it: Postgres sorts NULL
  last already, and an ordering that had to say `nullif(label, '')` would be a read undoing the way
...
Every table an operator names carries one, and the declaration says so rather than leaving it to be
...
A table nobody names is outside the rule and carries no label at all: there is no name for an unset
one to fall back to, so the column could only ever render a blank. The exceptions, which are tables
an operator names that still carry no label, are these, with the reason each one gives:
...
engine builds them from ([#729](https://github.com/hyperscaleav/omniglass/issues/729)), and every one
...
`label`, or the acronym list, leaves the estate exactly as it was until
