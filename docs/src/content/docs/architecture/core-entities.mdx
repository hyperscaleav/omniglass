...
- **label**, an optional friendly string a human reads ("HQ Boardroom DSP"). Absent is normal:
  `crestron` needs no label, and a tag never has one. Absent has exactly one spelling, the empty
  string: the column is `NOT NULL DEFAULT ''` on every table that carries it, so no projection has
  to decide whether `NULL` and `''` mean the same thing
  ([ADR-0118](/architecture/decisions/#adr-0118-the-friendly-string-an-operator-reads-is-a-label-and-unset-is-the-empty-string)).
  A surface with no label renders the name **verbatim**, never re-cased or prettified. That one
  spelling has a consequence worth stating where the rule is: Postgres sorts `NULL` last and `''`
  first, so a registry list ordered by the label spells its unset tail out,
  `order by nullif(label, '') nulls last, name`, rather than letting the column's nullability decide
  which end of the list the unlabelled rows land on.
