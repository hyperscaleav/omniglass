...
The rule language is Go `text/template` over a **closed map of strings** and a closed set of four
functions (`title`, `upper`, `lower`, `slug`). The map is the sandbox: a component's rule can read
`Name`, `Ordinal`, `TypeName`, `TypeAbbrev`, `Stem`, `ProductName` and `VendorName`, and there is no
mechanism by which any other name resolves to anything, so a secret is unreachable because it was
never put in rather than because a filter refused it. A rule that does not parse is refused at
