...
- a **system** delete **unbinds its members** (the `system_member` rows) but keeps the components,
  standalone and re-homeable. The unbinding is the same act `RemoveMember` performs, and it carries
  the same two consequences: a component left holding exactly one membership has that one promoted
  to its default, and every released component's platform-owned label restamps, because a rule reading
  `.SystemTypeLabel` reads the default membership's type and the default just moved;
...
**The console can ask for a generated name, and shows what it will be**
...
read, and then renders the SHAPE of the name a blank field will be filled with: the resolved stem,
with the ordinal written as the token `n` rather than a digit, because the ordinal is allocated
against live siblings inside the create's own transaction and does not exist until the row does. There
is deliberately no draft-preview verb that mints and rolls back: its answer would be provisional
(another create can take the number in between), and the rolled-back mint takes the advisory lock real
creates need. The label is not previewed at all, since its data map carries the row's own `Name` and
`Ordinal`. The placement bucket is shown beside the field as a path, never as a prefix inside it: a
name is unique within its placement rather than across the estate, so it no longer carries its
ancestry.
...
the stem (a system's `system_type` chain, a location's `location_type` rule) and the placement
bucket. So a reclassify that actually changes the classification re-mints, and a `:move` that
actually changes the bucket re-mints; a patch that re-states the classification and a move that
re-states the placement change neither input and leave the name exactly where it is. The distinction
is not tidiness: a lower ordinal freed by an earlier `:rename` means a spurious re-mint does not
recompute to the same answer, it MOVES the name, under `:update` or `:move` with no rename asked for
and possibly no `:rename` grant held. A system's bucket comparison is the three-way one (a parent
wins over a location, so relocating a parented system changes no bucket); a location's is the
parent alone.
