...
posts it back as `expected_ordinal`, never as a name (a posted name would claim the pen), the create
allocates under its own lock exactly as it always did, and one that would land a different number is
refused with a **409** naming the number that moved. The form re-reads the draft, shows the new name,
and the operator submits again. A refusal is honest where a silent renumber is not, and it is the
only outcome the locked field's promise admits.
