...
- A **node** is the edge process (`omniglass node run`) that pulls work, reaches components over
  endpoints, and ships results ([nodes](/architecture/nodes/)). It is structural because it is a
...
- A **`vendor`** (Boreal, Polaris, ...) names an organization, with a **`kind`** of `manufacturer`,
...
OpenAVCloud AV Device Taxonomy v1.1 ([ADR-0131](/architecture/decisions/#adr-0131-the-device-taxonomy-is-oavc-and-the-demo-catalog-is-fictional)): category roots, subcategory types,
...
A **`product`** (Kestrel VRoom, Boreal Edge 55, ...) is the concrete **SKU**: a stable `id` and
...
- **Canonical vocabulary** (the [`metric_type` and `property_type` catalogs](/architecture/properties/),
  ...
  a fleet still shapes its own place vocabulary, through the
...
- **name**, the renameable machine identifier an operator types and a URL, a CLI argument, and an
  address carry (the `rm215a` in `boi.17c.rm215a`). Renameable is the point: `kestrel-vroom` is a
...
- **label**, an optional friendly string a human reads ("HQ Boardroom DSP"). Absent is normal:
  `boreal` needs no label. Absent has exactly one spelling, **SQL NULL**:
...
friendly string beside it, so `tag`, `variable`, `secret` and `endpoint` went without a label, no
