...
import RuleLanguage from '../../../components/RuleLanguage.astro';
...
- A **location** ties systems and components to a physical place, classified by `location_type`, its
  only shape-definer (no template). A starter type set ships seeded and platform-owned, an
  operator's change to one held as a fork over it
  ...
  unconstrained, a non-empty set is enforced on create and move. The shipped set and its parents are
  rendered from the seed on the [location types guide](/guides/admin/location-types/) rather than
...
([#695](https://github.com/hyperscaleav/omniglass/issues/695)). Both fields ship because they answer
...
The shipped taxonomy, rendered from the seed, with a blank cell meaning the row inherits that fact
from its nearest ancestor:

<SeededSet kind="component_types" />
...
form ([ADR-0107](/architecture/decisions/#adr-0107-a-create-that-writes-a-membership-costs-what-the-membership-route-costs)).
...
- **Example content** (a `standard`) is created by **forking an in-code template**
...
- **Canonical vocabulary** (the [`metric_type` and `property_type` catalogs](/architecture/properties/),
  ...
  `product`) ride that path today, and `location_type` joined them in
  [ADR-0106](/architecture/decisions/#adr-0106-a-location-type-is-platform-owned-and-a-nullable-object-clears-under-the-mask):
  an estate still shapes its own place vocabulary, through the
  [registry fork](/architecture/decisions/#adr-0095-an-operator-forks-a-shipped-registry-row-instead-of-the-platform-writing-it)
  rather than by owning the row, which is what lets a release WITHDRAW a shipped value.
...
on every picker change without joining the queue every create in the estate is waiting in. There is
...
A draft answers only for a bucket the caller could create in. Omitting the parent names the
parentless bucket, which every create refuses without an all-scoped grant, so the draft refuses it
with the same **403**: the previewed ordinal is read from the bucket's sibling names, so answering
would report which of that bucket's names are taken, for whatever stem the caller put in a type's
name rule.

The placement bucket is shown beside the field as a path, never as a prefix inside it: a name is
unique within its placement rather than across the estate, so it no longer carries its ancestry.
...
bucket); a location's is the parent alone. And a component is the tier whose classification is not
its stem: it names a **product**, which points at a `component_type` chain, so two products under one
type mint the same name and its reclassify compares the resolved stem rather than the product it was
handed.
...
shipped place type carries one**, so a shipped estate names every location by hand: a campus, a
...
shipped estate, and reached when an operator declares a positional type of their own.
...
because it was never put in rather than because a filter refused it. The grammar is the sandbox for
...
The functions are the engine's own, rendered here from its one declaration rather than restated, so
a function this table names is a function the FuncMap defines and the grammar admits
([#701](https://github.com/hyperscaleav/omniglass/issues/701)):

<RuleLanguage />

<RuleLanguage builtins />
...
`boardroom`; the [decision log](/architecture/decisions/#adr-0101-the-first-of-its-stem-in-a-bucket-carries-no-ordinal-and-the-mint-that-says-so-is-the-one-allocation-tests)
...
estates are full of: a divisible boardroom is two `board` systems in one room, and both rendered
...
restamps-nothing rule applies as for locations: an estate that already exists keeps both halves
