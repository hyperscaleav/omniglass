---
title: Core entities
description: "The fleet model: component, system, location, and node as the structural entities, the variable-depth trees, and the exclusive-arc owner."
sidebar:
  badge:
    text: Partial
    variant: note
...
**A name is unique within its placement, not across the whole fleet**
...
stay fleet-wide unique. A bare-name reference that now matches more than one row is refused
...
:::design[The `global` fleet owner (ADR-0057)]
Above the four sits a singleton **`global`** fleet owner where fleet-wide health and KPIs roll up:
...
- **Canonical vocabulary** (the [`metric_type` and `property_type` catalogs](/architecture/properties/),
  ...
  an fleet still shapes its own place vocabulary, through the
...
unique within its placement rather than across the fleet, so it no longer carries its ancestry.
...
own**. A new fleet gets a rendered label when each location is created; an fleet that already exists
...
restamps-nothing rule applies as for locations: an fleet that already exists keeps both halves
