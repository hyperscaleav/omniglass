...
positional type (a floor, a rack unit) will need.
...
A **location** carries the pen and both verbs and generates nothing: `location_type` has no stem, so
`:resetName` on a location refuses (422) with the missing fact named rather than reporting a reset
that did not happen. The pen ships ahead of the generator on purpose, so a location an operator
names today is already frozen when a positional type learns to name its own rows.
