...
`TypeName`, `TypeAbbrev`, `Stem`, `ProductName` and `VendorName`, and there is no mechanism by which
any other name resolves to anything, so a secret is unreachable because it was never put in rather
than because a filter refused it. The grammar is the sandbox for what a rule can DO: an allowlist
