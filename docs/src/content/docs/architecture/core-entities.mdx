...
system (see [System roles](#system-roles-the-slots-a-system-needs-filled) below).
...
estate-wide health and KPIs roll up. One per deployment, no FK. No built table
...
*without a system in hand*, a default and never a rule that overrides a caller who named a system (the
cascade page owns that contract). A component's first membership takes the default with nobody asking,
and a partial unique index makes a second primary impossible rather than merely unlikely.
...
-> card). The trees are the structural backbone of the [cascade](/architecture/cascade/), which
resolves over an entity's containment path; the precedence rules are stated once, there.
...
This nesting feeds two mechanisms: the [cascade](/architecture/cascade/), where the deeper node wins,
and the **health rollup**, where a **sub-system's health rolls into its parent system** and a
**sub-component's into its parent component**, the same role-aware composition that runs up the rest
of the tree.
...
What stays single-valued is **which system chain seeds the cascade**. Why the seed must be
single-valued, how `is_primary` picks it for context-free callers, and the per-system effective views
that answer the genuine "config differs per system" case are stated once, on
...
`system_member` is **built** as the plain binding above, without the frozen template BOM. The **built** slot is
