...
`slug`, `words` plus `and`/`or`/`not` and the six comparisons, and refuses everything else text/template
...
Because a rule change's blast radius is the whole estate, **shipping this rule restamps nothing on its
own**. A new estate gets a rendered label when each location is created; an estate that already exists
