...
[ADR-0063](/architecture/decisions/#adr-0063-the-telemetry-model-is-typed-registries-over-bare-noun-data-tables) confirms the separation this page describes. The **`event_type` registry** is built: list, get, and custom-type create/update/delete over `/event-types`, seeded official types (`syslog.line`, `call.started`), operator-extensible, official rows read-only, mirroring the `property_type` catalog. The **`event`** occurrence now carries **`origin`** (caught/caused/derived/scheduled) plus the `caused_by_event_id` and `correlation_id` causation columns, and **log-to-event promotion** is built for the caught path: with the `log` kind gone from `property_type`, a registered `event_type` name routes an ingested occurrence to the event sink with `origin=caught`, under the same owner-confinement and reject-not-project as a metric or state. The **`caused`** origin now has a producer: a [command](/architecture/commands/) records a caused `command.issued` event on issue (ADR-0063 #396). Still directional: the `event_rule` engine and the `derived`/`scheduled` producers, the alarm edge, and the CDC publisher.
:::
