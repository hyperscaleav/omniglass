...
**provenance** (observed, calculated, intended, declared) and a **source**, and any two
...
*compare* intended and declared values, and never infer a new fact from an intended
value treated as truth (see [The DAG invariant](#the-dag-invariant)).
...
- **Provenance** answers *how do we know this particular value?* It varies per **row**.
  The same `power.state` can be observed, intended, or declared at different moments.
...
- **`datapoint_type`** describes every datapoint key: `(namespace, name, kind,
  value_type, unit, authoritative_provenance, fusion_policy)`. One registry across all
...
- **`*_type`** (`component_type`, `system_type`, `location_type`), classification +
  a **field schema** (the declared-datapoint keys instances carry) + **type-level
...
- **The instance** (`component`, `system`, `location`), pins a template version
  (component/system) and carries its own declared values.

**Properties are declared datapoints.** A "field" on a type/template is a
declared-datapoint key with a default; the instance value is a `state_datapoint` with
`provenance=declared`. There is no separate property/attribute store, inventory like
`model` or `asset_tag` is simply a declared datapoint with no observed counterpart (so
divergence never fires on it), and it gets audited change history for free.

### The current-state cascade

The effective declared/default value of a key is resolved by walking from broadest to
...
Provenance is stamped per datapoint row. The same key, same value, can be known four
...
| **declared** | asserted by an operator (config, identity, inventory) | `audit_id` (written same-tx with `audit_log`) |

A value of any provenance is still a metric/state/log (the kind is fixed by the key);
provenance records *how it got there*. All four land in the same datapoint tables, side
by side for one key, which is what makes divergence detection free. The lineage CHECK
...
### declared: operator-asserted, replaces the prop tables

mac, ip, serial, locked-input, model, capacity, anything an operator sets is a
**declared** `state_datapoint`, written same-tx with its `audit_log` row. This dissolves
the old `prop_type` / `prop_binding` / `util_routing_props` machinery. Ownership
resolution (the transform `route` step) reads indexed **declared identity** datapoints
(serial/ip) to bind incoming telemetry to entities.

### authoritative_provenance: which wins on disagreement

A per-key attribute deciding which side wins when two provenances disagree:
**observed-wins** for facts (reality is truth) or **declared-wins** for config/identity
(the declaration is the spec; reality should conform, the self-healing pattern). Fixed
per key.
...
id against the component's **declared** identity (the stable anchor), not against another
source's observed serial (which dodges the serial-is-also-conflicting trap); a source's
observed serial is then validated against declared via `disagree`. **Reconciliation** is
...
- **`transform_rule`**: telemetry to datapoint: **match** (by task) then **extract**
  ...
  kind) then **route** (resolve the owning entity from indexed declared identity) then
...
- **`discovery_rule`** *(deferred, tracked)*, observed data **creates**
  components/systems/locations + their declared identity. Active/inactive;
...
> intended or declared values against observed (drift). It may **not** treat an intended
...
- `disagree(declared, observed)` means the world drifted from intent (config drift, swap)
...
because "current observed" and "current declared" are different values for one key, and
...
| **state_datapoint** | Categorical/text/object datapoint. Discrete, dwell-measurable. Holds declared config too. |
...
| **datapoint_type** | Registry for datapoint keys: namespace, name, kind, value_type, unit, authoritative_provenance, fusion_policy. Official/private shadow. |
...
| **provenance** | How we know a value: observed, calculated, intended, declared. Per row. |
...
| **declared** | Operator-asserted config/identity/inventory. Lineage: `audit_id`, same-tx with `audit_log`. A `state_datapoint`; replaces the prop tables. |
| **lineage (on-row)** | A derived row carries its own lineage; no execution table. The rule version is the backtest hinge. |
| **authoritative_provenance** | Per-key: which provenance wins on disagreement (observed-wins for facts, declared-wins for config/identity). |
...
| **discovery_rule** | *(deferred)* observed data creates components/systems/locations + declared identity; active/inactive; official/private; pairs with the authoritative source. |
...
| **cascade** | Resolves the effective declared/default value: global → type → template → location → system → component → group (weighted); most specific wins. |
...
| **audit_log** | Who-did-what ground truth; one row per operator write, same-tx; the lineage target for declared. |
