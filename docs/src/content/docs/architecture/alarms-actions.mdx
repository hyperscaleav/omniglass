...
It is **one incident, a new row per open**, keyed by
...
[CDC publisher](/architecture/messaging/) then publishes the committed
...
The built `alarm` row below is component-local: no owner arc, no `event_rule`, and no ack columns
yet. The **target design shape** adds `event_rule`, the owner arc, `status`, `opened_at`,
`resolved_at`, and `acked_by`. Either way it is a stateful entity, **one incident, new row per
open**, holding current state directly (not event-sourced); the ITSM anchor. History = events plus
audit by `id`.
