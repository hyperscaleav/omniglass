...
`action` is **still Design** (no `action` table exists today): a stateful entity carrying delivery
and step state (`id`, ordered `steps` of notify/command/wait/branch, `status`, `current_step`),
driven by events and alarms.
