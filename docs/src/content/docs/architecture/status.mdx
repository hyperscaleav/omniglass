...
- **Identity: password policy + generator.** A single pure validator (`auth.ValidatePassword`) enforces a
  password policy on **every** set-path: create a user, self-service change-password, CLI `set-password`, and
  `bootstrap`. The policy is **at least 12 characters, not on an embedded common-password denylist, and not
  ...
  (`generatePassword`, a readable charset with no look-alike characters, revealed for the admin to read) plus a
  **Copy** button. Proven by an `internal/auth` unit test (short / common / contains-username / valid), an API
