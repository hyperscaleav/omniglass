...
- **Collection slice (checkpoints 1-2): the storage tier and the node runtime.** The first cut of the
  ...
  port, plus a fake-seam unit test and the full enroll -> pull -> heartbeat round trip. Deliberate thin cuts
  ([ADR-0016](/architecture/decisions/)): a node is its own table (not a `principal`), the credential is a
  shared secret (nkey/JWT deferred), and the control plane is JSON over core NATS. Deferred to later
