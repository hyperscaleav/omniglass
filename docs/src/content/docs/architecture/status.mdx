...
- **Identity: grant scope operators (`scope_op`).** The `exclude_root` boolean generalizes into a `scope_op`
  ...
  read/create, == old `exclude_root=true`), and **`self`** (exactly the root row, no descendant walk, uniform
  across actions) which is **net-new**, a grant on exactly one node. The pure `scope.Resolve` gains a `SelfIDs`
