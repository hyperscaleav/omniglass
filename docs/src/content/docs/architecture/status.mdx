...
- **Identity: principal groups (grant-by-group), backend.** A `principal_group` plus membership, and a grant
  ...
  cannot group-grant owner). The console Groups page and the inherited-vs-direct grant view on the user detail
  are the next slice.
- **Collection slice (checkpoints 1-2): the storage tier and the node runtime.** The first cut of the
  ...
  Deliberate thin cuts ([ADR-0016](/architecture/decisions/)): the credential is an interim shared secret
...
- **Collection slice (checkpoint 3): the reachability datapoint, end to end.** The capability-primitive heart:
  ...
  **confines** a node to its own tasks (an Event carrying another node's `task_id` is orphan-dropped, no row
  ...
  and `node` binaries. Deliberate thin cuts ([ADR-0017](/architecture/decisions/)): the hot-path/async split
...
- **Collection slice (checkpoint 5a): the reachability verdict as a state.** The first honest substrate for
  ...
  See [ADR-0018](/architecture/decisions/#adr-0018-the-reachability-verdict-is-a-built-in-state).
...
- **Collection slice (checkpoint 5d-ui): the interface and task authoring surfaces.** The console gains the two
  ...
  list endpoint, the type picker offers the built types (`icmp`, `tcp`) this slice ships; a future
  `GET /interface-types` registry route can replace the static options. The **Add reachability check** button on
  the Reachability panel (gated on `interface:create` *and* `task:create`) authors a check the way the node runs
  one: it creates the interface (`type` = the chosen protocol, owned by that component, `params.target` the
  `host[:port]` the probe dials) then a **poll** task over it (`mode=poll`, enabled), then invalidates the
