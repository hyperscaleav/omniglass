...
- **The declared-property contract and value bodies carry `property_id`** beside `property_name`, the last
  ...
  event body's `key` (a telemetry key that is the property name under a different field) stays name-only for
  now, a noted exception: it cannot share the `property_id` -> `property_name` pairing and a high-volume
  event read is fine with the name.
