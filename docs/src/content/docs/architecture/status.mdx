...
- **Identity: admin password reset.** An administrator can reset another user's password without the
  ...
  the reset), policy-enforced (a 422 on a weak password), behind the same **takeover guard** as
  impersonation (an owner cannot be reset by anyone, and a caller cannot reset a principal whose
  capabilities exceed their own, a shared `checkTakeoverGuard`), and audited with the admin as the actor (the
  storage method `SetPrincipalPassword` sets by id, distinct from the self-service `SetPassword` that keys
  on username and audits the target). The console adds a **Reset password** kebab action on the user blade
  ...
  password is a 422, the admin reset lets the user sign in with the new password), and a web test (the
  kebab opens the panel, Generate + Set password calls the endpoint and confirms).
