...
- **Secret: the cascade-resolved encrypted value.** The first of the [config, credentials, and
  ...
  list, create, delete), `GET /components/{name}/effective-secrets` (the masked cascade view), and
  `POST /secrets/{id}:reveal` (the audited decrypt). Masked reads ride the viewer floor; create and delete are
  gated by `secret:create` / `secret:delete`, and **reveal** by the sensitive `secret:reveal`, which the
  `*:read` floor does not carry, so only admin (`secret:*`) and owner may decrypt. The console adds a **Secrets**
  directory (with an in-place reveal + copy) and, on a component's detail, the **effective-secrets** list: each
  resolved secret opens a blade that reveals the value and shows the full cascade of how it won (the winning tier
  and the candidates it shadowed). Renamed **credential to secret** (ADR-0017).
  Proven by a real-crypto testcontainer test (seal, jsonb round-trip, unseal; encrypted-at-rest; full cascade
  precedence), an HTTP e2e (the cascade, the authz split, and the reveal round-trip plus its 403), and the web
