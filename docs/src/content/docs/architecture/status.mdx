...
- **Collection slice (checkpoint 5b): the reachability surface, seen.** The first datapoint read surface. A
  ...
  **Reachability panel** on the component detail: one row per interface with a verdict pill (`responding` /
  ...
  transitions (up/down over time, not a latency trend), and an expandable L3/L4/L7 gate breakdown with the
  reason a down interface is down. It renders only real API fields. The availability-percentage SLI
