...
- **Catalog: the `component_model` product registry, and the make-in-use delete guard.** Slice 2 of
  ...
  `back_image_id` is a 422, and `make_id` is fixed after creation. This slice also lands the
  ...
  (CRUD, the FK faults, official read-only, the make-in-use 409), a dev-seed idempotency test, a
  real-binary API test (the CRUD lifecycle, unknown-make and unknown-image 422s, the make-in-use
  409, the capability gate), and web unit and component tests (the Make column, the make filter, the
  image-upload round-trip). Deliberate thin cuts: **no type or classification field** (the genus
