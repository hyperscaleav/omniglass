...
- **Identity: the auth event log.** The `audit_log` already recorded every privileged mutation (with `actor`
  ...
  **as <admin>** tag on anything done while impersonating. It is **admin/owner-only**: `audit` is a sensitive
  ...
  impersonated rows). Deferred: a failed-login rate-limit counter and rich filtering UI.
