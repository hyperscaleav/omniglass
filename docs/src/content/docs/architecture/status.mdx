...
- **The `property` latest-value cache and the reconciliation read land ([ADR-0063](/architecture/decisions/#adr-0063-the-telemetry-model-is-typed-registries-over-bare-noun-data-tables), first feature slice).**
  ...
  **non-gating** (a failed upsert is logged, never fails the ack, and the cache is rebuildable from the
  ...
  [datapoints](/architecture/datapoints/) and [config, secrets, and variables](/architecture/variables/)
...
- **The event_type family lands: events get their own registry ([ADR-0063](/architecture/decisions/#adr-0063-the-telemetry-model-is-typed-registries-over-bare-noun-data-tables), second feature slice).**
  ...
  gated `event_type:*`, seeded official `syslog.line` and `call.started`, official rows read-only), a
  generated CLI and typed client, and a console **Event Types** catalog page mirroring Properties.
  **Log-to-event promotion** is built for the caught path: with `log` gone from `property_type`, a
  registered `event_type` name routes an ingested occurrence to the `event` sink with `origin=caught`, under
  the same owner-confinement and reject-not-project as a metric or state, and the events panel shows the
  origin. Only the caught path has a producer today; `caused` is the command pillar (#396) and
  `derived`/`scheduled` need the rule engine and the clock (the action layer).
  [events](/architecture/events/) advances `Design` to **`Partial`**; [datapoints](/architecture/datapoints/)
