...
- **Tag: filter the directories by their effective tags.** The last major tag piece, closing the
  set-and-see loop's read half: the Components, Systems, and Locations chip filters gain **one facet per
  tag** in use, so an operator narrows a directory by any tag exactly like a built-in field. A tag facet
  ...
  **value-less** operators, **is set** (`?`) and **is absent** (`!?`), that test only whether the tag is
  ...
  projects one `FilterKey` per tag key present on the loaded rows. `ListConfig.filterKeys` becomes
  **accessor-reactive** (a `FilterKeys<T>` is an array **or** an accessor, resolved in a reactive scope), so
  the facet set appears the moment the effective tags resolve and tracks whatever the estate is tagged with;
  each page merges its static facets with a `tagFacets` memo derived from the rows. Deferred: server-side tag
  filtering at pagination scale (today the match is client-side over the loaded rows) and a stored per-key
  color override ([#226](https://github.com/hyperscaleav/omniglass/issues/226)). Proven by a `lib/predicate`
  unit suite (the presence operators, the per-key facet projection and its dedup against the static keys, the
  value-less tokenizing) and a `FilterBar` render test (a `?` / `!?` token commits a value-less chip that
  renders with no value button).
