...
- **Collection slice (checkpoint 5a): the reachability verdict as a state.** The first honest substrate for
  ...
  See [ADR-0033](/architecture/decisions/#adr-0033-the-reachability-verdict-is-a-built-in-state).
...
- **Collection slice (checkpoint 5f): reachability lands read-only; check authoring deferred.** Before the PR, the
  ...
  reachability check is authored from the raw **Interfaces** and **Tasks** pages (create an interface, then a poll
  task over it), which stay as low-level primitives. The slice lands as **remote node reachability**: enroll a
...
- **Collection slice (checkpoint 5g): the derived interface/task model.** The reachability primitives were
  reframed to their honest shape ([ADR-0035](/architecture/decisions/#adr-0035-the-task-is-derived-read-only-plumbing-projected-from-its-interface)):
  ...
  **named by its protocol** (the name derives from its `interface_type`, unique per component), so the create
  ...
  `ON DELETE CASCADE`). The console follows: the standalone Tasks page and its `/tasks` route are removed and a
  node's derived tasks read as a **panel on the node detail**, the interface create form drops the name input, the
  interface and node detail move onto the shared read-edit-save blade, and the collection pages route through the
