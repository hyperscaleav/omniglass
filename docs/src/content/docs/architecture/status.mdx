...
- **The raw-log ingest lane is built for the read path ([ADR-0066](/architecture/decisions/#adr-0066-logs-are-a-raw-ingest-lane-not-events)).**
  ...
  client, and a console **Logs** panel beside the events panel (severity badges, structured fields on
  demand). The dev seed installs the lobby display's device logs (link, CEC, EDID, input, thermal) as log
...
- **The raw-log lane gets its first producer: node self-logs ([ADR-0066](/architecture/decisions/#adr-0066-logs-are-a-raw-ingest-lane-not-events)).**
  ...
  that writes to its console and buffers every record; the run loop drains it each tick and publishes the
  ...
  **Self-logs** panel on the node blade. The dev seed installs the edge node's example self-logs. The
