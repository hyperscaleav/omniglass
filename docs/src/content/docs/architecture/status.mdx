...
- **Identity: an admin can view and revoke a user's sessions (slice 2 of 2).** An administrator can now
  ...
  **Users** blade gains **Sessions** and **API tokens** sections (a shared `SessionsList` now backs both
  them and the self-service Profile card), hidden unless the caller holds the capability, with a **Revoke** per row. The
  generated CLI gains `omniglass principal sessions <id>` / `principal revoke-session <id> <sid>` (the
  cligen `nameOverride` seam groups them under `principal` so they do not collide with the self-service
  `session` commands, which share the leaf noun). Proven by a real-binary API test: an admin lists a target's
  sessions and revokes one so it stops authenticating, an operator is 403, revoking an **owner's** session
  as a lesser admin is 403 (the takeover guard), a `sid` that is not the target's (or an unknown principal)
  is a 404, the secret never appears, and the revoke is audited with the admin as actor.
