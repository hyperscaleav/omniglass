...
- **Tag: the governed label vocabulary and its cascaded bindings.** The [tag](/architecture/tags/) primitive lands as
  ...
  `PATCH|DELETE /tags/{name}`, `PUT|DELETE /tags/{name}/binding` (the global value), `GET|PUT|DELETE
  /{components,systems,locations}/{name}/tags[/{key}]`, and `GET /components/{name}/effective-tags` (the per-component
  cascade). Deferred (ADR-0021): the console surface (a Tags directory and per-entity editor), binding via
  [groups](/architecture/groups/) and a `template` default ([#184](https://github.com/hyperscaleav/omniglass/issues/184)),
  value-domain governance, and binding onto a file. Proven by a pure `internal/tag` validation unit test (key
