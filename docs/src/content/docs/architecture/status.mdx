...
- **Identity: every credential is time-bounded, and sessions split from API tokens.** Building on the slice
  above, the tokens-never-expire behavior is reversed ([ADR-0017](/architecture/decisions/#adr-0017-every-credential-is-time-bounded-token-purpose-not-expiry-shape)):
