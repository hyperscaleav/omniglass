...
- **Secret: the cascade-resolved encrypted value.** The first of the [config, credentials, and
  ...
  list, create, delete), and `GET /components/{name}/effective-secrets` (the masked cascade view); read rides the
  viewer floor, create and delete are gated by `secret:create` / `secret:delete` (admin `secret:*`, owner `>`).
  The console adds a **Secrets** directory and the **effective-secrets** panel on a component's detail (the
  pedagogical view of which value wins where). Renamed **credential to secret** (ADR-0017).
  Proven by a real-crypto testcontainer test (seal, jsonb round-trip, unseal; encrypted-at-rest; full cascade
  precedence), an HTTP e2e (the cascade and the authz split), and the web data-layer suite. Deferred: the
  **variable** and **config** members of the page, the reveal endpoint and the interpolation consumer, secret
  **lifecycle** (oauth2 refresh, rotation), and groups / weighted precedence in the cascade.
