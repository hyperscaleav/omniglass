...
- **Console: the read -> Edit -> Save contract on a blade footer action bar.** Every detail blade opens
  ...
  **footer action bar** that `BladeStack` renders: a destructive action on the left (always available), secondary
  ...
  (one backend action per Save) remain deferred to a batched endpoint; the user deactivate / purge lifecycle is
...
- **Identity: the principal lifecycle (deactivate + purge), backend.** Beyond the reversible disable, a principal
  can be **deactivated** (a soft delete, `deactivated_at`: hidden from the directory, cannot authenticate,
  reversible) and then **purged** (an irreversible hard delete, gated on prior deactivation and on the
  ...
  retiring the "never hard-deleted" rule). The last-active-owner guard extends to deactivate. Gateway
  (`DeactivatePrincipal` / `ReactivatePrincipal` / `PurgePrincipal`), the `:deactivate` / `:reactivate` /
  ...
  split, the deactivate-before-purge gate, soft-delete visibility). The console UI for the lifecycle is a
...
- **Identity: the principal lifecycle in the console, UI.** The user blade's footer action bar presents the
  lifecycle by state: the left slot is the reversible toggle (Disable / Enable, or Reactivate for a deactivated
  account), and the kebab holds the escalating red steps (Deactivate when live, Purge when deactivated and the
  caller holds `principal:purge:admin`), each confirmed. The detail fetches the principal by id (not the
  directory list, which hides deactivated) so a just-soft-deleted user stays resolvable, and a **Show
  deactivated** toggle on the Users directory (a new `include_deactivated` list param) surfaces hidden accounts
  to re-find one. `BladeSecondary` gains a red tone and the destructive slot a restore tone; the web `can()`
  already honours the three-token `principal:purge:admin`. Proven by the web suite (the live and deactivated
  footer states and the show-deactivated flow) and a storage test for the include-deactivated list; completes
