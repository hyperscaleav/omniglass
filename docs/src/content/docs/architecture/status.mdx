...
- **Identity: IAM console form + blade hardening, UI.** A polish pass over the identity surfaces. Removing a
  principal now closes its blade cleanly: a **purge** (and a group **delete**) closes the blade first, drops the
  dead detail query, then refreshes the list, so the blade never lingers on a 404 of the entity it just removed.
  ...
  **New group** (it leaked the `principal_group` resource key), the group create form gained the same
  subtitle the user form has, and the form placeholders are consistent across create and edit. Proven by an API
