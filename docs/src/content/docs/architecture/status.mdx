...
- **UI: the keyboard-shortcut registry (slice 0).** The console gains its first shortcut **primitive**,
  ...
  command palette grows a **shortcut-hint column** with a "Keyboard shortcuts" action. No new user-facing
  bindings and no storage change this slice. Proven by the keymap-core unit suite (grammar and platform
