...
- **Catalog: the `component_make` manufacturer registry.** The first landed slice of a larger
  ...
  anchor. The console ships a **[Makes](/guides/admin/makes/)** catalog page (`FlatList` + blade,
...
- **Field: an operator-defined typed attribute on a type (slice 0).** The first cut of the
  [field](/architecture/variables/#field-an-operator-defined-typed-schema-on-a-type) primitive: an operator declares a typed
...
- **Platform: the settings engine (global level).** The [settings](/architecture/settings/) subsystem ships its first
  slice: a pure `settings` package that resolves an effective configuration document by deep-merging ordered layers (embedded
  `code` defaults, an optional operator `file`, and the `global` DB override) most-specific-wins in JSON map-space, tracking
  ...
  ([ADR-0032](/architecture/decisions/#adr-0033-settings-persist-only-the-override-level-base-layers-are-recomputed-in-memory)),
  and its Gateway methods are **unscoped**, gated by the `settings:<action>` permission alone
  ([ADR-0033](/architecture/decisions/#adr-0034-the-settings-gateway-is-unscoped-only-the-permission-gates-it)), reusing the
  [cascade](/architecture/cascade/) primitive on the principal axis
  ([ADR-0034](/architecture/decisions/#adr-0035-settings-resolve-as-a-cascade-over-principals-with-a-broader-wins-lock)). The
...
- **Config, secrets, and variables: the per-component effective-secrets and effective-variables panels retire.**
  ...
  [field](/architecture/variables/#field-an-operator-defined-typed-schema-on-a-type) epic
...
- **Node tags (N2).** A node becomes a taggable owner kind, alongside global / component / system /
  ...
  an all scope (`node:update`, reusing the identity gate) and its effective tags are the global layer
  ...
  Proven by a storage integration test (the applies_to gate, the all-scope requirement, effective = global
...
- **The unified key registry.** The `datapoint_type` catalog is generalized into the primitive-agnostic
  **`canonical_key`** registry (the physical table; the concept, the `/keys` API, the Go `Key`, and the
  console all read `key`): the typed keyspace a datapoint **observes** and a field **declares**. The
  `(scope, name)` ladder collapses to a `name` primary key plus an **`official`** boolean (seed-owned keys
  read-only); `value_type` becomes **`data_type`** over `{string, int, float, bool, json}` (`text`
  backfills to `string`, `bool` added); **`kind`** is nullable (a declared-only attribute key has none);
  ...
  **`internal/key`** primitive holds the one canonical name-format rule (lowercase, dot-hierarchied) and
  the typed value validator; tag's key rule folds onto it. The Storage Gateway gains custom-key CRUD
  (gated `key:create` / `:update` / `:delete`, official keys read-only, audited in the same transaction),
  exposed at **`/keys`** with a generated CLI and client, and a new **Catalog > Keys** console page (list,
  blade, create form). An official seed ships the reachability keys plus a starter attribute set
  (`serial_number`, `mac_address`, `firmware_version`, `model_number`). Proven by `internal/key` unit
  tests, the `canonical_key` CRUD integration test, the `/keys` HTTP e2e (the `key:create` 403 and official
  read-only), and the console suite. The type-schema (`field_definition.key`, PR-B) and reconciliation are
  deferred ([ADR-0043](/architecture/decisions/#adr-0043-the-unified-key-registry),
...
- **A field declares a key (the type-schema binding).** The deferred half of the key registry lands: a
  `field_definition` gains a **`key`** FK to `canonical_key`, and its `name`, `data_type`, and
  `display_name` now come **from the key** (denormalized onto the row so the effective-fields read and the
  value validation need no join). The create surface picks a key instead of typing a free name and type;
  the default is validated against the key through **`internal/key.ValidateValue`** (its data type plus its
  JSON Schema), so an out-of-enum or out-of-range default is refused at the definition. The `key` column is
  **nullable** (a legacy field keeps rendering; the migration backfills `key = name` where a matching key
  exists), while a new create **requires** a registered key (`ErrUnknownKey` to 422). The console
  field-on-type editor picks a key with a new reusable **`KeyPicker`** (a searchable Kobalte Combobox over
  `/keys`, portaled, generic for the driver epic) and gains **per-row edit and delete** for a declared
  field, closing the gap where a field had no edit or delete. Proven by the storage integration tier (the
  key FK, the derive-from-key identity, the JSON-Schema default validation), the `/field-definitions` HTTP
  e2e (declare from a key, unknown-key 422), the `KeyPicker` unit tests (search, filter, exclude, portal,
  onSelect), and the field-editor console tests. The type-schema editor for how a type selects **many** keys
  at once, re-keying a field, and value-against-schema validation are deferred
  ([ADR-0044](/architecture/decisions/#adr-0044-a-field-declares-a-key),
  [#297](https://github.com/hyperscaleav/omniglass/issues/297)). The
  [config, secrets, and variables](/architecture/variables/) page stays `Partial` (the field cascade is the
  remaining floor to `Built`).
