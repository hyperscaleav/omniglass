...
against the `property_type` registry. The observed **state** path now has its first producer too: the node
...
Calculated provenance, fusion, and the full two-lane data plane are still design. The wire shape is
...
**The promotion ladder is template -> org -> official.** Each step is a cheap **re-scope or re-point**, not a migration: lift a template's `input` to an org-canonical `video.input` and re-point the template's sample at it; later it gets blessed **official** by being shipped in the distribution (the one real way trust is earned, not a flag an operator sets). Samples already collected keep resolving. This is the "shift to normalized over time" path.
...
Provenance is the second axis, stamped per sample row. The same key, with the same value, can be known three ways. Each provenance points at the immutable ground-truth record that produced it (its **lineage**), and the lineage column populated is mutually exclusive per provenance, enforced by a CHECK constraint.
...
mac, ip, serial, locked-input, anything an operator *sets* is declared intent, and declared intent is **not** a sample provenance. It lives in [config](/architecture/variables/): keyed to the same canonical signal as its observed side, resolved through the scope cascade, never in the sample tables. There is no separate property store: config is the declared side of a signal plus the cascade. Ownership resolution reads the resolved identity (a declared identity config value, or the observed identity sample that shares its key) to bind observed data to components, through the [identity-binding index](/architecture/collection/) (a `(property_type, value) -> owner` arc) that collection maintains.

### Precedence: spec versus status lives in config

When declared intent and observed reality disagree, which one wins is a **per-config-item `reconcile` policy** ([config](/architecture/variables/#drift-and-reconcile)), not a per-key sample attribute:
...
Among sample provenances there is no precedence contest: intended is a pending bet that observed confirms or refutes (reconciliation, see [intended](#intended-the-declared-effect-of-a-command)), and observed supersedes it on arrival. The spec-versus-status decision is config's reconcile policy, not a per-key sample attribute. Device-swap (where a declared MAC is briefly authoritative before the device reports it) is handled by a component's [maintenance mode](/architecture/core-entities/#operational-mode-active-maintenance-disabled), which suppresses drift.

## The current-value cache

The sample tables are append-only history: every observation, forever. Asking *what is this
property now?* over them is an `order by ts desc limit 1` whose cost grows with retention. The
**`property` cache** answers that read in one indexed row.

`property` holds the newest value per **series** `(owner, property_type, instance, provenance)`,
one row, upserted on intake. It is the value store, not a sample table, so caching a producer value
here never adds a row to `metric` / `state`. Three provenances are cached; one is not:
...
failure is logged, not retried into the ack**. The cache can lag by consumer-lag and is rebuildable
from the samples, and because the upsert is idempotent on the series key, a redelivery re-runs it
harmlessly. An **out-of-order guard** on the conflict update, keyed off the value's own `ts` (not the
row-write time), keeps a late-arriving old sample from displacing a newer value:
...
`metric` and `state` are untouched, they stay the append-only history for trends and audit. The
cache decouples *the current value* from *all the samples*.

## Reconciliation: want, told, is

The read side pivots three values per property of an owner:

- **want** = declared, resolved live from the cascade.
- **told** = intended, a cache lookup (nil until a command opens it).
- **is** = observed, a cache lookup.

**Drift** is `is` present and disagreeing with `want`, computed on read so it cannot go stale, the
same [`disagree`](#disagree-and-divergence) comparison applied to a property that holds more than one
provenance. Declared is resolved live inside the pivot (the cascade resolver, reused not duplicated),
so `want` is always the true current config even though it was never written into the cache. The
[command pillar](/architecture/commands/) adds the **windowed** form: settlement is `told` vs `is`
gated on the command's settle window (pending inside it, settled on a match past it, failed otherwise).
...
The command -> device -> observed-sample round trip is the one path where the acyclic structure cannot be read off the static graph (a command can provoke an observed value that fires the rule that issued the command). The propagated `correlation_id` closes that gap: because the command stamps its id onto the observed sample, the run that fires off that observed value carries the same thread, and the cycle-guard walk follows a **real carried id** across the round trip rather than inferring lineage. The DAG invariant is therefore enforced, not merely assumed, on the only edge that needs runtime help.
...
Current value (latest per owner / key / **instance** / **provenance**, reduced across the source perspectives per the effective `fusion_policy`) is a **view** over the persisted tables, correct and zero-maintenance. It is keyed per-provenance because "current observed power" and "current intended power" are different values for the same key, and the divergence model depends on seeing both. A materialized `current_value` table is a measured optimization, earned when a read profile proves the view too slow: the driver is **operator and fleet-dashboard reads**, not the rule engine, since the rule engine evaluates against samples live off the JetStream `samples` stream and never reads the view. The same view-by-default discipline as storage applies. Ownership resolution reads resolved identity config (the declared value, else the observed [identity sample](/architecture/collection/)) by targeted indexed lookup, not a full scan, so it does not by itself justify the materialized table.
