...
(`POST /principals`) and **update** (`PATCH /principals/{id}`: label, email, username), the
...
The console **Roles view** (`GET /roles`, gated `role:read:admin`) lists these read-only with label, description, inheritance, and **effective permissions**. The role blade renders a **net** view: the **permission universe** ([below](#the-permission-universe-published-per-route)) plus, per role, the **held** subset (resolved server-side by the same `rbac.Set.Allows` matcher), one-per-line, lexicographically, with a `Held / Missing / All` toggle. Custom-role editing is a later slice.
...
GET /api/v1/auth/me
{
  ...
  "human":     { "username": "jordan", "email": "jordan@example.com", "label": "Jordan Rivera", ... },
  "permissions": [
...
<SchemaTable
  table="human"
  notes={{
    label: 'The person&#39;s real name, the operator-facing label',
  }}
/>
