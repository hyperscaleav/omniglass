...
assignment** (`POST /principals/{id}/grants` / `DELETE /principals/{id}/grants/{grantId}`) with the **owner-invariant trigger** enforcing
...
(the `/principal-groups` family: CRUD, membership, and group grants) whose members
...
stored base64 on the human row). The per-slice breakdown is on [implementation status](/architecture/status/).
...
:::design[Custom-role management, tracked in #434]
### Custom roles
...
A permission a role **grants** but the universe does **not** contain is a capability that is not yet routed (enforces nothing). Since #463 reversed the ship-ahead policy there are none: a grant lands in `roles.yaml` in the same slice as the first route that enforces it, never before, and the **drift test**'s `aheadOfRoutes` allow-list is deliberately empty, so a seed grant that gates nothing turns the build red the day it appears. The docs lint closes the loop from the other side: a permission named in prose must gate a route too, or sit inside a design fence.
...
**Worked example (per-grant binding denies estate-wide ack).** Principal P holds two grants: `operator @ group-A` (role carries `alarm:ack`) and `viewer @ all` (read-only). Alarm X is owned by a component in **group-B**. P calls the illustrative `POST /alarms/X:ack`:
...
:::design[The permission cache, tracked in #434]
## Caching strategy
...
`permissions` is the flat union of the caller's roles' **raw patterns** (wildcards like `principal:*` and `>` ride through unexpanded, and the `:read` floor is not materialized), so a `useCan(...)` check in the web app applies the same pattern-matching rules the server does. It is a **fast-reject / UI hint only**, the union over all grants: it answers "could this principal ever do X anywhere", never "can it do X to **this** entity". List visibility likewise (a row in `GET /components` is read-scoped) does **not** imply per-action authority on that row. Per-row action affordances (the ack/snooze button on a specific alarm) must be computed against `visible_set(P, action)` for that target, which the `grants` array drives: `grants` is the source for advanced UI logic (scope chips, deciding per-row actionability, explaining why a button is or is not shown). The server is the only authority regardless; the flat list and the list view are hints, the scoped gateway decides.
...
Base `principal` is identity (an opaque uuid) plus `kind` only; the per-kind tables hold the rest,
including each kind's label: `human.display_name` (the person's real name) plus username and email,
the `service` label, and the `node` name (whose full table renders on
