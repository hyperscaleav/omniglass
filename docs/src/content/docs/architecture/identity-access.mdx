...
- **The write path** (`resolveScopedRef`, used to resolve a create's or an action's placement or owner
  reference) keeps the **notFound-versus-forbidden split** a write already makes elsewhere: absent
  anywhere is 404, present only outside the given scope is 403, because the caller supplied this
  reference itself in the same request, not the new disclosure a read's own uuid would be. Its
  guarantee is narrower than it can sound: a resolved reference is one the caller may **write** a
  binding against, not one the caller may **read**, and a bare-name 403 is a name-existence oracle by
  construction (a caller learns a name exists somewhere in the estate from the status code alone, with
  no candidates disclosed), predating this epic and tested (`interfaces_scope_test.go:82`), now
  asymmetric with the read path's single 404 by design, not by omission.
