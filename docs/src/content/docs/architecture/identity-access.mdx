...
routes. They are separate namespaces with no crossing, which is why the fleet-side storage methods are
...
Today each fleet entity is scoped by its own tier (a `location` scope contains locations only, and
so on; only `secret` / `variable` / `endpoint` / `task` resolve through their owning entity's
...
A **scope** says *how much of the fleet* a grant reaches; `platform:<action>` says whether the principal may
...
2. **Scope resolved from the acknowledgement's own permission**: `visible_set(P, alarm, acknowledge)` unions only the grants whose role carries it, so it is `{component-A}`; the fleet-wide `viewer` grant carries no acknowledgement and widens nothing. This is the load-bearing part: resolving the scope from `component:read` (which P holds everywhere) or from `component:update` (which a pure responder role need not hold at all) would answer the wrong question in opposite directions.
...
- **The write path** (`resolveScopedRef`, used to resolve a create's or an action's **same-tier**
  ...
  **write** a binding against, not one the caller may **read**, and a bare-name 403 is a
  name-existence oracle by construction (a caller learns a name exists somewhere in the fleet from
  the status code alone, with no candidates disclosed), predating this epic and tested
  (`endpoints_scope_test.go:82`), now asymmetric with the read path's single 404 by design, not by
  omission.
- **The cross-tier placement bind** (`resolvePlacementRef`, used by `CreateComponent`,
  ...
  candidates**: it was the one bind that redacted them while resolving fleet-wide, and it stopped
