...
- **The cross-tier placement bind** (`resolvePlacementRef`, used by `CreateComponent`,
  ...
  against) takes the **read path's policy on the referenced tier**: the caller's `location:read` /
  `system:read` scope decides, and out of scope is the same non-disclosing 422 an absent reference
  gives. Neither of the two policies above fits it. The write policy's set is resolved for the entity
  ...
  caller; and the question a placement asks is a **read** question, because since
