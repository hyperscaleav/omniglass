...
| `deploy` | viewer + create/update on locations, systems, and components (the integrator / field-tech role, typically granted with the `subtree_excl_root` operator to build out a subtree without editing its root). No delete. **A grant of it reaches one tier, the tier it is scoped at:** the permission set names three tiers, but a location-kind grant fills no system-tier or component-tier scope at all (not even to read one), because a scope kind contains only its own tier until the cross-tier expansion lands ([#10](https://github.com/hyperscaleav/omniglass/issues/10)). Building out a building therefore takes a location-scoped, a system-scoped and a component-scoped grant of this role together, which is the shape `internal/api/placement_scope_e2e_test.go` uses; the reach is pinned by test in `internal/seed/role_reach_test.go`, and the role's own description says so. Carries the same scoped `secret:read,reveal,create,update` as `operator`, for the device secrets a field tech sets up during a site build. |
