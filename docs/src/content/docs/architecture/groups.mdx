...
`/principal-groups` CRUD gated by `principal_group:*`, and the console Groups pages). Note the
distinction: the built group is a grant **subject** (members inherit the grants targeted at the
group), while group as a **scope** (`scope_kind=group`) is refused today with a 422.
:::
...
The general `group` table carries the id, the kind (component/system/location/principal), the
...
A group is placed on the cascade's one specificity scale by its **weight**, the only weights in the
system: a high weight sits above deployment (a must-apply override), a low weight below it (a default
that deployment overrides). The scale itself, the segmented precedence key behind it, and the worked
example are stated once, on
[cascade](/architecture/cascade/#groups-overlay-the-cascade-placed-by-weight).

## Multiple membership

An entity belongs to a flat **set** of groups; there is no second precedence axis. However many groups
an entity is in, their bindings fold by weight on that one shared scale
([cascade](/architecture/cascade/#groups-overlay-the-cascade-placed-by-weight)), and the
...
A `_type` (device/app, AV-System, room) is a classification attribute, never a tree position: instead
of a type layer in the tree, you author a dynamic group filtered on type (`type == X`) and place it by
weight ([cascade](/architecture/cascade/#groups-overlay-the-cascade-placed-by-weight)).
