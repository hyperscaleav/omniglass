...
- **`file`** is **indexable metadata**: name, content-type, size, `sha256` (no `tags` column yet,
  deferred to [#191](https://github.com/hyperscaleav/omniglass/issues/191)), the searchable handle
  an operator references; it owns no bytes, pointing at a blob by hash.
