...
`file` tags are deferred; when they
...
floor (`*:read`) reads ordinary files; only the flag fences the confidential ones.

:::design[The resource-classification and clearance lattice, tracked in #243]
When the [resource-classification + clearance](https://github.com/hyperscaleav/omniglass/issues/243) lattice lands,
...
`file` (**Partial**) is the searchable metadata handle; it points at a blob by hash. Tenant-wide (no
placement arc); `sensitive` lifts the row to the `:admin` tier and defaults false. Tags-on-files is
[#191](https://github.com/hyperscaleav/omniglass/issues/191).
...
`bytea`), S3 / disk behind the same `blob.Store` seam. Content type lives on the file, not here,
because content-addressing is about the bytes. A file delete frees its unreferenced blob synchronously
(dedup-aware); async mark-sweep GC of aged or event-referenced blobs is deferred.
