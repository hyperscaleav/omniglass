...
- **Amended a third time (#743): the mark reaches the LIST, where it trails the value.** Everything
  above shipped on the blade alone, so the console's two surfaces contradicted each other about one
  fact two clicks apart: the Stem and Abbrev cells rendered an em dash on exactly the rows whose
  blade showed the value they inherit. Those cells now render the inherited value, marked with the
  same dot, through one `InheritedCell` primitive shared by both registries. Nothing is fetched for
  it, since `inherited_*` already rides the listing that is also the blade's read.
  - **Beside the VALUE here, which scopes the placement rule above rather than reversing it.**
    "Beside the label" was decided for a FIELD, whose label occupies one slot across read and edit. A
    table's label is in the HEADER, one per column instead of one per row, so there is no per-row
    label to share and the two-state argument has nothing to bite on. The mark trails the value,
    because leading it steps every marked value out of a column the unmarked ones hold and reads as a
    bullet list, which is the same frame that ruled leading out of a blade's read state.
  - **The dot is the only distinction, because dimming is already spent.** The blade tells an
    inherited value apart by muting it beside a stated value that is not muted. Every value in these
    columns is muted already (`text-base-content/60`, the secondary-column treatment), so dimming
    would have to either brighten every stated value, a change to the rows that are not the subject,
    or push the inherited ones to `/40`, which is what this console gives an ABSENT value: that would
    make "comes from elsewhere" look like "nothing here", the exact confusion being fixed.
  - **The Icon cell is brought along, not kept as precedent.** It has rendered `resolved_icon` since
    #695 with no distinction at all, showing an inherited glyph exactly as a stated one. Once the two
    columns beside it tell the two apart it is the only one left conflating them, so it takes the
    same mark, keeping `resolved_icon` as its TEXT (what the row shows) and reading
    `inherited_icon_source` for its ancestor, which are the same answer on any row stating no icon.
