...
### ADR-0043: The unified key registry

- **Date:** 2026-07-19 | **Status:** Accepted | **Pages:** [config, secrets, and variables](/architecture/variables/)
- **Decision:** The `datapoint_type` catalog is generalized into a primitive-agnostic **`canonical_key`**
  registry: one typed keyspace whose entry (a **key**) is the canonical, typed name that a datapoint
  **observes** and a field **declares**. The physical table is `canonical_key` (`KEY` is a SQL keyword);
  the concept, the API resource (`/keys`), the Go `Key` type, the console, and the `field_definition.key`
  column all read `key`. Four shape changes fold `datapoint_type` in: the unused `(scope, name)` ladder
  ...
  **`data_type`** over the unified set `{string, int, float, bool, json}` (`text` backfills to `string`,
  `bool` is added); **`kind`** (metric/state/log) becomes nullable, since a declared-only attribute key
  has no observed kind; and **`validation`** is a **JSON Schema** fragment (`pattern`/`enum`/`minimum`/
  `maximum` and, for a json key, a nested object schema), enforced by Huma's own validator with the stored
...
- **Deferred:** the `field_definition.key` reference (PR-B); the type-schema editor (how a `component_type`
  selects keys); reconciliation (the declared-versus-observed drift signal); a console editor for the
