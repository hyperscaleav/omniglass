...
:::note[Partial: the content-addressed blob store (pgblobs backend), the `file` handle CRUD (create-from-upload with sha256 dedup, get, list, download, delete), the per-file `sensitive` tier, and the Files directory under Values are built ([ADR-0029](/architecture/decisions/#adr-0029-files-slice-1-a-content-addressed-blob-store-and-a-tenant-wide-file-handle)); attachment, GC, alternate backends, and tags-on-files are deferred]
