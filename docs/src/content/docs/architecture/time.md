...
- **`timer`** (mechanism, working-set): every *pending* fire, kind-discriminated
  ...
  JetStream; a watchdog's staleness enters the data lane as a derived sample); rows are then
...
- **No-data is derived.** The gap is *already in the recorded data* (the absence of sample rows
...
- **`stale`**: we *had* a value and it has aged past its expected cadence. The watchdog's product
  ...
  **actionable**, because a signal that stopped most often means lost visibility (the source
  died). The watchdog emits a derived staleness sample (`X stale at T`, and `fresh again` on
  resume).
...
a went-stale property is the `stale` reason, and a covered-but-never-reported property is the
