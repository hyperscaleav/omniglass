...
[cascade](/architecture/cascade/) like config and variables, but sealed so its value never sits in the clear. Its first slice is **built**
...
tuning constant. These are Zabbix-style **macros**, resolved down the same
[cascade](/architecture/cascade/) and interpolated as `$var:<name>` into functions, interface
definitions, and rule scopes.
...
exactly like config and variables; the [tags](/architecture/tags/) page owns the combinator and the
[cascade](/architecture/cascade/) the precedence. A [template](/architecture/templates/) binds its own
tags onto its component (`category: audio-dsp`), and an inherited tag is a practical scoping dimension:
a high-weight [group](/architecture/groups/) can key a rule-set off `compliance: pci`, an action can
read a `maintenance_window`.
...
- **The cascade.** One resolver serves all four kinds, and the structural chain, the precedence key,
  and the fall-through to a declaration default are stated once, on
  [cascade](/architecture/cascade/); tags ride it with their own union-on-key, override-on-value
  combinator ([tags](/architecture/tags/)).
...
`official` marks shipped-canonical versus org-local, like the sample and role registries.
...
`secret` (**Built**) is the encrypted cell and the `$sec:` cascade key; scope is the exclusive arc without its system arm (ADR-0052).
Read masked through a **scope-filtered** directory; decrypted only through the audited `:reveal` /
`:copy` path. Visibility is placement scope plus the per-secret `admin_sensitive` flag (admin-only
when set); `secret` is off the `*:read` floor
...
`value_type` in the app), no observed side. The **config** cell (declared/observed/reconcile) is a
separate, deferred member.
