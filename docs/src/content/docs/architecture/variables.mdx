...
- **Names are org-specific config keys, not canonical signals**, the one place the
  "operator-defined, not curated" namespace applies: no registry, no pre-registration; sprawl is
  controlled by a creation **role-gate** ([IAM](/architecture/identity-access/)) and by every
  variable being **surfaced in the tree**.
