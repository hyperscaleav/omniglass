...
below); interpolation is a **`$sec:name.path`** token in an endpoint input or function arg, never
...
`tls-cert` `notAfter`) warns proactively, and **observed-use failure** flips it unhealthy after N
...
interpolated as `$var:<name>` into functions, endpoint definitions, and rule scopes.
...
- **A name is org-specific, not a canonical signal**, the one place the "operator-defined, not
  ...
  holds, though (one segment of lowercase letters, digits, and hyphens, no dot), so `newtron-ssh`
  names a variable and `newtron.ssh` is refused. Sprawl is controlled by a creation **role-gate**
