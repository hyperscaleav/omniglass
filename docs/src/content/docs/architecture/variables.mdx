...
the built surfaces being the Secrets and Variables **directories** plus the **property** slot that consumes a
...
**declared** value you set, sharing the **key** but not the **storage** (the declared value lives in
the config table, resolved down the cascade, **never a sample row**). Their gap is **drift**.
...
Adopting the observed value as the declared one is **not** an ongoing mode; it is a separate
**one-shot import action** an operator runs deliberately.
...
fills that role; drift and reconcile then just happen.
...
[cascade](/architecture/cascade/), sealed so its value never sits in the clear
...
path), not a sensitivity flag on ordinary values; within it, the **binary** `admin_sensitive` flag
below splits admin-only platform credentials from operator-visible device secrets.
...
- The `/secrets` **directory is scope-filtered** and **hides admin-sensitive rows** from a caller
  without the admin tier, so a platform credential's existence never appears.
...
- **Names are org-specific config keys, not canonical signals**, the one place the
  "operator-defined, not curated" namespace genuinely applies: no registry, no pre-registration;
  sprawl is controlled by a creation **role-gate** ([IAM](/architecture/identity-access/)) and by
  every variable being **surfaced in the tree**.
...
Three pieces are **built**. The **`property_type` catalog**: the primitive-agnostic registry of typed names
(`name`, `data_type` over `string` / `int` / `float` / `bool` / `json`, optional label, unit, a JSON Schema
`validation`, and a nullable observed `kind`), seed-owned `official` rows read-only
([ADR-0043](/architecture/decisions/#adr-0043-the-property-catalog)). The three **classifier contracts**
(**`product_property`**, **`standard_property`**, **`location_type_property`**), one shape: a catalog property
with an optional `default_value` and a `required` flag, unique per `(classifier, property)`. And **`property`**,
the value store on the **same owner exclusive-arc** as the sample sinks and `event`, with an `instance`
discriminator and a **`provenance`**; the write path fills `provenance=declared`. The read is
...
**deferred**, listed plainly so a built badge never hides drift
...
model): when a `state` lands whose `(owner, key)` a config item is keyed to, it refreshes that item's
...
`state` **kind** is unchanged (an observed `power.state = on` is still a `state`); only the
*declared* value moved. The spec-and-status loop gets a real home instead of overloading sample
...
**`property`** value store on [properties](/architecture/properties/); **`tag`** and
