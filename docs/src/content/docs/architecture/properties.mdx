...
The **`unit` registry is still Design** (no table exists today): one canonical unit per family
...
resolves same-family targets through it. `precision` as a display hint rides the same future slice.
