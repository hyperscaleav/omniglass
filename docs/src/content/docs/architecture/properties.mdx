...
kinds beyond `component` are still directional (#422). The wire shape is
...
Samples are the **data lane**: NATS-native, consumed live by the rule engine, with the persistence
consumer batch-writing the durable copy as an async sink. Events, alarms, and actions are the
**record/state lane**: Postgres-first, CDC-out. The two-lane model, both consumers, and the ingest
paths are described once, on [messaging](/architecture/messaging/#two-lanes-one-bus); this page owns
what a sample *is* and how it lands.
:::
...
key on one owner stay distinct series.

:::design[The authored instance form, tracked in #430]
The authored form for the extractor engine, its instance-identity decision pending as an ADR under
[#430](https://github.com/hyperscaleav/omniglass/issues/430): the instance rides the pipeline as
...
- **Live on NATS, durable in PG.** The live sample is the message on the JetStream `samples` stream; the durable copy in the `metric` / `state` tables is the **persistence consumer**'s async sink write, idempotent on series identity, and samples **do not go through CDC** ([messaging](/architecture/messaging/#two-lanes-one-bus)).
...
`metric` is the firehose (BRIN on ts, numeric aggregation); `state` is sparse and transition-only
(time-in-state and dwell), the table [config](/architecture/variables/) keys its observed side to.
Trace columns (`correlation_id` and a causation parent) live on `event` / `log_line` today;
extending them to samples is design.
...
`property_type` is the key registry across the sample kinds (log occurrences moved to the
[`event_type`](/architecture/events/) registry, ADR-0063). The name is globally unique and
`official` decides authority; the template/org/official `scope` ladder with `template_id` is future
design.
...
The end-to-end pipeline diagram, both lanes on one bus, lives on
[messaging](/architecture/messaging/#the-pipeline-end-to-end).
