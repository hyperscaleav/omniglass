...
| **version-range pin** | A `system_template_member` binding a role to a semver range over one template lineage (e.g. `>=2.1 <3.0`); reuses the channel/latest mechanism bounded by the range. The degenerate capability contract over a single lineage. See [templates](/architecture/templates/). |
| **capability contract** | A static interface a `system_template_member` may pin a role to for cross-vendor substitutability: required commands + required canonical datapoint_types (+ `health_role`), written in canonical terms. Conformance is a set-containment check (a component's template capability-set superset-of the required-set), evaluated at bind / re-pin time and re-checked only when the component's template version or the contract changes. Not continuous evaluation, not a matching engine, not Expr. See [templates](/architecture/templates/). |
