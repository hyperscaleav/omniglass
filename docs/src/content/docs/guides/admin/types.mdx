...
the **name** (the operator-facing kebab handle, for example `wing`), **display name**, and
**origin** (**official**, seed-owned, or **custom**); the Location tab's rows also carry an
**icon** glyph key. A type also carries an `id`, a uuid minted by the database, the internal
address the handle resolves to ([ADR-0062](/architecture/decisions/)); the handle is what you
type and read.

- Switch tabs to move between registries; **name** matches a kebab handle or display name within
  the active tab, and **official** narrows it to official or custom rows.
- **New type** (with `type:create`, an admin permission) opens a create **drawer** scoped to
  ...
  slice): give it a **name** (the kebab handle, unique within that kind, e.g. `wing`), a
  ...
  **unconstrained** (any parent, or root), the default; `root` is a reserved **name** no real type may
  take.
- Pick a row to open its **detail blade**. The footer **Edit** pencil (with `type:update`) edits
  the display name, the icon, and the allowed parents; the kind and the **name** are fixed, since a
  catalog row carries no rename (`:rename` is a component, system, location, and principal group
  affordance). **Delete** (with `type:delete`) removes the row, behind a confirm.
- **The shipped location types are yours to change.** They arrive as ordinary **operator-owned**
  rows (origin **custom**, not official), so relabel them, re-parent them, or delete the ones your
  estate does not use. They are forked from a template in the code, once, with no inheritance, and
  the boot seed installs them **only if absent**, so a restart never reverts your edits. See
