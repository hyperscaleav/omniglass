...
the **name** (for example `wing`), **display name**, and
...
- Switch tabs to move between registries; **name** matches a name or display name within
...
- **New type** (with `type:create`, an admin permission) opens a create **drawer** scoped to
  ...
  slice): give it a **name** (unique within that kind, e.g. `wing`), a
