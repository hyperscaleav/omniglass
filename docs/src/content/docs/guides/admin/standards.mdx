...
operator-facing name, for example `huddle-room`), the **display name**, its **parent
...
- **New standard** (with `standard:create`, an admin permission) opens a create drawer: give it a
  **name** (unique tenant-wide, e.g. `lecture-hall`) and a **display name**, and
...
- Pick a row to open its **detail blade**. The footer **Edit** pencil (with `standard:update`) edits the
  display name and parent; the **name** is fixed, since a catalog row carries no rename. **Delete** (with
...
`omniglass standard role update <id> <role> --display-name <label> --quorum <n> --accepted-types <types>
