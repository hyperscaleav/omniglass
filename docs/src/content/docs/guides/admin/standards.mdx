...
operator-facing kebab handle, for example `huddle-room`), the **display name**, its **parent
...
- **New standard** (with `standard:create`, an admin permission) opens a create drawer: give it a
  **name** (the kebab handle, unique tenant-wide, e.g. `lecture-hall`) and a **display name**, and
...
- Pick a row to open its **detail blade**. The footer **Edit** pencil (with `standard:update`) edits the
  display name and parent; the uuid `id` is database-minted and never edited. **Delete** (with
