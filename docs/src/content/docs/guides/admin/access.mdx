...
directly, or to a **group** the user belongs to; the scope decides which slice of the fleet the
...
and Groups: a directory row per role (its **name**, whether it is **official**, what it inherits, and how many
...
The same matrix, rendered here from the seed itself (`make gen`) with inheritance, wildcards, and
the read floor resolved exactly as the authorizer resolves them at runtime:
...
**New group** creates one (name, label, description); the **name** is an ordinary entity name
(lowercase letters, digits, and hyphens, no dots and no underscores), while the label is free
text. The name is not patchable once the group exists: moving it is `POST /principal-groups/{id}:rename`,
gated by `principal_group:rename` rather than `:update` ([ADR-0077](/architecture/decisions/)), and the
console's group blade shows the name read-only. The new group then opens
...
From the CLI the surface is `omniglass principal-group list` / `get` / `create` / `update` / `rename` /
`delete`, plus its
...
descendants and no adding children under it), so you can grant a field tech the rooms inside a
building without letting them rename the building, or lock an operator to a single node. A scope
kind reaches its own tier only, so a field tech who also works the systems and components in that
building needs a grant on each of those tiers beside the location one. Removing an existing grant marks it (dimmed and
