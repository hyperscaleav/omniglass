...
- **New property** (with `property_type:create`, granted to operators) opens a create **drawer**: name
  the **key** (lowercase kebab, dot-hierarchied, for example `serial-number` or
  ...
  **description**, **unit**, and **kind**. Leave the kind as **declared** for an operator-set
  attribute. An invalid key (an uppercase letter, an underscore, a leading hyphen, a stray dot) is
