...
**Catalog > Telemetry > Properties** (with `property_type:read`, covered by every viewer's `*:read` floor) is
...
- A property is a **value** (what something *is*: an input, a firmware version, a reachability
  ...
  ([**Catalog > Telemetry > Metrics**](/guides/admin/metrics/)), carrying the numeric facts (unit, precision).
...
|---|---|---|---|
| a **product** | its components | **Catalog > Components > [Products](/guides/admin/products/)** | `product:update` / `:delete` |
| a **standard** | the systems that conform to it | **Catalog > Systems > [Standards](/guides/admin/standards/)** | `standard:update` / `:delete` |
| a **location type** | the locations of that type | **Catalog > Locations > [Types](/guides/admin/location-types/)** | `location_type:update` / `:delete` |
