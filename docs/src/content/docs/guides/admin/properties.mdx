...
**property** is a **name** plus a data type, with an optional display name, unit, and validation. The
name is its canonical identifier, a dot-hierarchied path of lowercase kebab segments, and the same
...
- The directory lists every property sorted by name, each showing its **name** (the canonical
  identifier, with its display name on the line above it), **type** (`string`, `int`, `float`, `bool`,
  or `json`), **kind**, and **origin** (**official**, seed-owned, or **custom**). **name** filters by
...
- **Kind** marks a property that is **observed** as telemetry: `metric` (a continuous measure),
  ...
  attribute, something an operator sets, like `serial-number`, that is never collected off a
  device.
- **New property** (with `property_type:create`, granted to operators) opens a create **drawer**: give
  it a **name** (lowercase kebab segments joined by dots, for example `serial-number` or
  `interface-reachable`), choose its **data type**, and optionally add a **display name**,
  **description**, **unit**, and **kind**. Leave the kind as **declared** for an operator-set
  attribute. An invalid name (an uppercase letter, an underscore, a leading hyphen, a stray dot) is
  refused with a message.
- Pick a row to open its **detail blade**. The footer **Edit** pencil (with `property_type:update`) edits
  the display name, description, and unit; the name, data type, and kind are fixed at creation, since
