...
The binary is configured by environment, BYO Postgres. The table renders from the declared
...
The node process reads its own small set (the CLI-scope variables surface through the
[CLI reference](/reference/cli/) instead):

<ConfigVars scope="node" />
