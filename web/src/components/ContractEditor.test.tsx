...
const contract: ClassifierProperty[] = [
  { property_name: "seat_count", property_type_id: "seat_count-id", required: true },
  { property_name: "display_count", property_type_id: "display_count-id", default_value: 2, required: false },
];
...
describe("ContractEditor on a standard", () => {
  ...
  it("declares a picked catalog property, PUTting to the standards contract route", async () => {
    ...
    vi.spyOn(globalThis, "fetch").mockImplementation(async (input) => {
      ...
      if (req.method === "PUT") {
        put = req.clone();
        return json({ property_name: "has_camera", property_type_id: "has_camera-id", default_value: true, required: false });
      }
  ...
  it("edits a line in place, PUTting the revised default", async () => {
    ...
    vi.spyOn(globalThis, "fetch").mockImplementation(async (input) => {
      ...
      if (req.method === "PUT") {
        put = req.clone();
        return json({ property_name: "display_count", property_type_id: "display_count-id", default_value: 3, required: false });
      }
      return json({ properties: contract });
    });
...
describe("ContractEditor on a location type", () => {
  ...
  it("speaks location-type language over the location-types route", async () => {
    ...
    vi.spyOn(globalThis, "fetch").mockImplementation(async (input) => {
      ...
      if (req.method === "PUT") {
        put = req.clone();
        return json({ property_name: "has_camera", property_type_id: "has_camera-id", required: true });
      }
      return json({ properties: contract });
    });
