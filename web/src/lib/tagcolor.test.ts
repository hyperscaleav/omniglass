...
describe("tagHue", () => {
  ...
  it("only ever returns a hue from the curated ramp", () => {
    for (const k of ["environment", "category", "cost_center", "compliance", "asset-id", "tier", "vendor", "room", "x", ""]) {
      expect(TAG_HUES).toContain(tagHue(k));
    }
  });

  it("spreads distinct keys across the ramp (not all one color)", () => {
    const keys = ["environment", "category", "cost_center", "compliance", "asset-id", "tier", "vendor", "room", "owner", "maintenance_window"];
