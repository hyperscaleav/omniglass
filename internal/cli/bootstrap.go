...
func newBootstrapCmd() *cobra.Command {
	var email, label, password string
	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			return runBootstrap(cmd.Context(), args[0], email, label, password, ttl)
		},
	}
	cmd.Flags().StringVar(&email, "email", "", "owner email (optional)")
	cmd.Flags().StringVar(&label, "label", "", "owner label (optional)")
...
func runBootstrap(ctx context.Context, username, email, label, password string, ttl time.Duration) error {
