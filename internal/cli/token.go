...
func newTokenCmd() *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: "Issues a new bearer credential for an existing principal, addressed by username, and prints " +
			...
			"fresh login token for `make dev` when the owner already exists. The token expires after --ttl " +
			"(default 90 days, hard maximum 365 days); every credential is time-bounded.",
		Args: cobra.ExactArgs(1),
		RunE: func(cmd *cobra.Command, args []string) error {
			return runToken(cmd.Context(), args[0], ttl)
		},
	}
...
func runToken(ctx context.Context, username string, ttl time.Duration) error {
	...
	// A CLI-minted API token is a bounded 'token' credential: no eternal secret.
	expiresAt := time.Now().Add(ttl)
	ok, err := gw.IssueBearerCredential(ctx, username, hash, prefix, "token", &expiresAt)
	if err != nil {
		return err
	}
	if !ok {
