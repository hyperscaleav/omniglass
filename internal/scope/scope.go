...
func applicableKinds(resource string) map[string]bool {
	...
	case "secret":
		// A secret is owned on the exclusive arc (location, system, or component),
		// so a grant scoped to any of those tiers can contain the secret's owner:
		// the gateway's inScopeTree walk resolves the specific owner within it.
