...
func TestReferencesResolveByEitherForm(t *testing.T) {
	...
	made, err := gw.CreateComponent(ctx, "", storage.ComponentSpec{Name: "codec"}, all, all, all, all)
	if err != nil {
...
func TestJoinFieldsAcceptEitherForm(t *testing.T) {
	...
	byName := "site"
	byID := site.ID
	a, err := gw.CreateComponent(ctx, "", storage.ComponentSpec{Name: "a", LocationName: &byName}, all, all, all, all)
	if err != nil {
		t.Fatalf("create by name: %v", err)
	}
	b, err := gw.CreateComponent(ctx, "", storage.ComponentSpec{Name: "b", LocationName: &byID}, all, all, all, all)
	if err != nil {
