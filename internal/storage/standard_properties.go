...
func upsertStandardPropertyRow(ctx context.Context, q querier, standardID string, spec StandardPropertySpec) (*StandardProperty, error) {
	...
	pp, err := scanStandardProperty(q.QueryRow(ctx, `
		insert into standard_property (standard_id, property_type_id, default_value, required)
		values ((select id from standard where `+registryRefCol(standardID)+` = $1), (select id from property_type where name = $2), $3, $4)
		on conflict (standard_id, property_type_id) do update
...
func (p *PG) ListStandardProperties(ctx context.Context, standardID string) ([]StandardProperty, error) {
	rows, err := p.pool.Query(ctx, `select `+standardPropertyCols+` from standard_property where standard_id = (select id from standard where `+registryRefCol(standardID)+` = $1) order by (select pr.name from property_type pr where pr.id = standard_property.property_type_id)`, standardID)
	if err != nil {
...
func (p *PG) DeleteStandardProperty(ctx context.Context, actorID, standardID, propertyName string) error {
	...
	before, err := scanStandardProperty(tx.QueryRow(ctx, `
		...
		where standard_id = (select id from standard where `+registryRefCol(standardID)+` = $1) and property_type_id = (select id from property_type where name = $2)
