...
func TestNodeGateway(t *testing.T) {
	...
	if _, err := conn.Exec(ctx, `insert into task (id, mode, interface_id, node_name, spec, enabled) values ('t-off', 'poll', (select id from interface where name = 'disp-1-icmp'), 'node-a', '{}'::jsonb, false)`); err != nil {
