...
// ComponentEndpoint is one placement-bound connection owned by a component: the
...
// layer signals, and history are read separately per (component, key, endpoint).
...
// ListComponentEndpoints returns a component's endpoints ordered by the label
// an operator reads with the unlabelled last (#613), the rows the reachability
// read composes over. It is the panel where two endpoints of one transport sit
...
// endpoints by the verified reference (name or uuid, ADR-0062), resolved once
...
func (p *PG) ListComponentEndpoints(ctx context.Context, componentRef string) ([]ComponentEndpoint, error) {
	...
	rows, err := p.pool.Query(ctx, `
		select i.name, coalesce(i.label, ''), i.transport, coalesce((select n.name from node n where n.principal_id = i.node_name), ''), i.params
		from endpoint i
		where i.component = $1::uuid
		order by i.label nulls last, i.name asc`, c.ID)
	if err != nil {
		return nil, fmt.Errorf("storage: list endpoints for %s: %w", componentRef, err)
	}
	defer rows.Close()
	var out []ComponentEndpoint
	for rows.Next() {
		var it ComponentEndpoint
		if err := rows.Scan(&it.Name, &it.Label, &it.Transport, &it.NodeName, &it.Params); err != nil {
			return nil, fmt.Errorf("storage: scan endpoint for %s: %w", componentRef, err)
		}
		out = append(out, it)
	}
	if err := rows.Err(); err != nil {
		return nil, fmt.Errorf("storage: iterate endpoints for %s: %w", componentRef, err)
	}
	return out, nil
}
