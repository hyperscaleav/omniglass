...
const capabilityCols = `id, display_name, official, created_at, updated_at`

func scanCapability(row pgx.Row) (*Capability, error) {
	var c Capability
	if err := row.Scan(&c.ID, &c.DisplayName, &c.Official, &c.CreatedAt, &c.UpdatedAt); err != nil {
		return nil, err
	}
	return &c, nil
}
...
func (p *PG) UpsertCapability(ctx context.Context, c Capability) error {
	_, err := p.pool.Exec(ctx, `
		insert into capability (id, display_name, official)
		values ($1, $2, $3)
		on conflict (id) do update
			set display_name = excluded.display_name,
			    official     = excluded.official,
			    updated_at   = now()`,
		c.ID, c.DisplayName, c.Official)
	if err != nil {
		return fmt.Errorf("storage: upsert capability %q: %w", c.ID, err)
	}
	return nil
}
...
func (p *PG) ListCapabilities(ctx context.Context) ([]Capability, error) {
	rows, err := p.pool.Query(ctx, `select `+capabilityCols+` from capability order by display_name, id`)
	if err != nil {
...
func (p *PG) GetCapability(ctx context.Context, id string) (*Capability, error) {
	c, err := scanCapability(p.pool.QueryRow(ctx, `select `+capabilityCols+` from capability where id = $1`, id))
...
func (p *PG) CreateCapability(ctx context.Context, actorID string, c Capability) (*Capability, error) {
	...
	if err := tx.QueryRow(ctx, `
		...
		return nil, fmt.Errorf("storage: insert capability %q: %w", c.ID, err)
	}
...
func (p *PG) DeleteCapability(ctx context.Context, actorID, id string) error {
	...
	if _, err := tx.Exec(ctx, `delete from capability where id = $1`, id); err != nil {
