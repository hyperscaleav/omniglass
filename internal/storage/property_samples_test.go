...
func TestInsertPropertySamples(t *testing.T) {
	...
	// Two transitions for one series (disp-1, endpoint-reachable, if-1): up then down.
	if err := gw.InsertPropertySamples(ctx, []storage.PropertySampleWrite{
		{OwnerKind: "component", OwnerID: "disp-1", Key: "endpoint-reachable", Instance: "if-1", Value: "up", Source: "tcp", TS: t0},
	}); err != nil {
		t.Fatalf("insert up: %v", err)
	}
	if err := gw.InsertPropertySamples(ctx, []storage.PropertySampleWrite{
		{OwnerKind: "component", OwnerID: "disp-1", Key: "endpoint-reachable", Instance: "if-1", Value: "down", Source: "tcp", TS: t1},
	}); err != nil {
		t.Fatalf("insert down: %v", err)
	}

	latest, err := gw.LatestProperty(ctx, "disp-1", "endpoint-reachable", "if-1")
	if err != nil {
		t.Fatalf("latest property: %v", err)
	}
	if latest == nil || latest.Value != "down" || latest.Provenance != "observed" {
		t.Fatalf("latest endpoint-reachable: want down observed, got %+v", latest)
	}

	// A different instance is a different series: no value yet.
	if other, err := gw.LatestProperty(ctx, "disp-1", "endpoint-reachable", "if-2"); err != nil {
		t.Fatalf("latest if-2: %v", err)
	} else if other != nil {
		t.Fatalf("if-2 should have no state, got %+v", other)
	}

	rows, err := gw.PropertyTransitions(ctx, "disp-1", "endpoint-reachable", "if-1", 0)
	if err != nil {
	...
	if err := gw.InsertPropertySamples(ctx, []storage.PropertySampleWrite{
		{OwnerKind: "component", OwnerID: "ghost", Key: "endpoint-reachable", Instance: "if-1", Value: "up", Source: "tcp", TS: t1},
	}); err == nil {
