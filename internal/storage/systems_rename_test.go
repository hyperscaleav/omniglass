...
func TestRenameSystem(t *testing.T) {
	...
	bad := "Bad Name"
	if _, err := gw.UpdateSystem(ctx, "", "av-child", storage.SystemPatch{Name: &bad}, all, all); !errors.Is(err, storage.ErrInvalidSegment) {
	...
	if _, err := gw.CreateSystem(ctx, "", storage.SystemSpec{Name: "Bad Name"}, all); !errors.Is(err, storage.ErrInvalidSegment) {
