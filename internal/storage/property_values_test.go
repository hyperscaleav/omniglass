...
func TestEffectiveProperties(t *testing.T) {
	...
	if _, err := gw.SetProperty(ctx, "", "component", "panel-1", "firmware-version", "", json.RawMessage(`"2.5.1"`), all, all); err != nil {
	...
	if _, err := gw.SetProperty(ctx, "", "component", "panel-1", "firmware-version", "", json.RawMessage(`"2.5.2"`), all, all); err != nil {
	...
	// off-contract.
	if _, err := gw.SetProperty(ctx, "", "component", "panel-1", "mac-address", "", json.RawMessage(`"aa:bb:cc:dd:ee:ff"`), all, all); err != nil {
	...
	// explicit miss.
	if err := gw.ClearProperty(ctx, "", "component", "panel-1", "firmware-version", "", all, all); err != nil {
	...
	if err := gw.ClearProperty(ctx, "", "component", "panel-1", "firmware-version", "", all, all); !errors.Is(err, storage.ErrPropertyNotFound) {
	...
	if _, err := gw.SetProperty(ctx, "", "component", "loose-1", "serial-number", "", json.RawMessage(`"SN-9"`), all, all); err != nil {
	...
	if _, err := gw.SetProperty(ctx, "", "component", "panel-1", "not.a.property", "", json.RawMessage(`1`), all, all); !errors.Is(err, storage.ErrPropertyRefNotFound) {
	...
	if _, err := gw.SetProperty(ctx, "", "component", "ghost", "serial-number", "", json.RawMessage(`"x"`), all, all); !errors.Is(err, storage.ErrComponentNotFound) {
