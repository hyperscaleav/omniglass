...
// an alarm or wrote an audit entry reads as blank; nothing seeds a node grant
// today, so there is no path that does it, and widening the resolution is its
...
// SQL NULL and nothing else, exactly as a coalesce reads it, so an unknown id, a
// purged principal and a node all come back empty and the two shapes agree on
// every input rather than only on the inputs the schema can produce.
//
