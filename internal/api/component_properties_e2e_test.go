...
func TestComponentPropertiesAPI(t *testing.T) {
	...
	c.do(ownerTok, http.MethodPut, "/components/disp-1/properties/serial-number",
		map[string]any{"value": "SN-123"}, http.StatusOK)
	sn = read(ownerTok, "disp-1").find(t, "serial-number")
	...
	c.do(ownerTok, http.MethodPut, "/components/disp-1/properties/mac-address",
	...
	mac := got.find(t, "mac-address")
	if mac.FromContract || mac.Required || !mac.IsSet || string(mac.Value) != `"aa:bb:cc:dd:ee:ff"` {
		t.Fatalf("mac-address = %+v, want an off-contract set value", mac)
	}
	if len(mac.DefaultValue) != 0 {
		t.Fatalf("mac-address default = %s, want none (no contract line)", mac.DefaultValue)
	}
	...
	c.do(ownerTok, http.MethodPut, "/components/nope-1/properties/serial-number",
		map[string]any{"value": "x"}, http.StatusNotFound)

	// Clearing the override falls back to the contract default; clearing it twice
	// is an explicit miss.
	c.do(ownerTok, http.MethodDelete, "/components/disp-1/properties/serial-number", nil, http.StatusNoContent)
	sn = read(ownerTok, "disp-1").find(t, "serial-number")
	...
	c.do(ownerTok, http.MethodDelete, "/components/disp-1/properties/serial-number", nil, http.StatusNotFound)
	...
	c.do(ownerTok, http.MethodPost, "/components", map[string]any{
		"name": "other-1", "product": "generic-device",
	}, http.StatusCreated)
