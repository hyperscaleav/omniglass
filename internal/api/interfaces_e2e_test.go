...
func TestInterfaceAPI(t *testing.T) {
	...
	// Owner (all scope + owner wildcard) runs full CRUD.
	c.do(ownerTok, http.MethodPost, "/interfaces", map[string]any{
	...
	c.do(ownerTok, http.MethodGet, "/interfaces/if-a", nil, http.StatusOK)
	...
	c.do(viewerAllTok, http.MethodGet, "/interfaces/if-a", nil, http.StatusOK)
	c.do(viewerAllTok, http.MethodPost, "/interfaces", map[string]any{"name": "if-x", "type": "tcp", "component": "comp-a"}, http.StatusForbidden)
	c.do(viewerAllTok, http.MethodPatch, "/interfaces/if-a", map[string]any{"params": map[string]any{"target": "9.9.9.9"}}, http.StatusForbidden)
	...
	c.do(opBTok, http.MethodGet, "/interfaces/if-a", nil, http.StatusNotFound)
	c.do(opBTok, http.MethodPatch, "/interfaces/if-a", map[string]any{"params": map[string]any{"target": "9.9.9.9"}}, http.StatusNotFound)
	c.do(opBTok, http.MethodPost, "/interfaces", map[string]any{"name": "if-a2", "type": "tcp", "component": "comp-a"}, http.StatusForbidden)
	c.do(opBTok, http.MethodGet, "/interfaces/if-b", nil, http.StatusOK)
