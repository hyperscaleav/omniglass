...
func TestTagAPI(t *testing.T) {
	...
	compRaw := c.do(ownerTok, http.MethodPost, "/components", map[string]any{"name": "codec-1", "system": "sys", "location": "room", "product": "generic-device"}, http.StatusCreated)
	var comp struct {
		ID string `json:"id"`
	}
	...
	c.do(opTok, http.MethodPost, "/tags/environment:setPlatform", map[string]any{"value": "x"}, http.StatusForbidden)
