...
func TestEventTypeAPI(t *testing.T) {
	...
	if !names["cable.unplugged"] || !names["syslog.line"] {
	...
	c.do(ownerTok, http.MethodPatch, "/event-types/syslog.line", map[string]any{"display_name": "x"}, http.StatusConflict)
	c.do(ownerTok, http.MethodDelete, "/event-types/syslog.line", nil, http.StatusConflict)
