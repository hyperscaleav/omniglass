...
type renderBody struct {
	...
	// task-15-review.md #2): resolving a component's own abbrev
	// (componentTypeIDForProduct plus resolveTypeFacts' own ancestor walk)
	// costs two more queries beyond PathOf's own, paid on every row of an
	// unpaginated LIST for a field no console surface reads. A LIST row's
	// bare still concatenates every segment (still a real, resolvable-by-
	// eye compact form), it just skips the substitution a GET applies.
...
// create/update/move/rename/resetName response: attachPath only runs on a
// GET or LIST fetch, see scopedConfig.attachPath's doc comment). A present
